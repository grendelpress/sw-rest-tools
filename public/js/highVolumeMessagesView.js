...
export class HighVolumeMessagesView {
    ...
    show(credentials) {
        ...
        if (!this.container) {
            ...
            document.querySelector('.workspace-main').appendChild(this.container);
        }

        // Hide all category views and overlay sections
    ...
    hide() {
        ...
        // Restore the current category view
        const activeNav = document.querySelector('.nav-item.active');
        if (activeNav) {
            const targetView = activeNav.dataset.view;
            document.querySelectorAll('.view').forEach(v => {
                v.classList.toggle('hidden', v.dataset.view !== targetView);
            });
        }
    }
        ...
        const applyFilters = () => {
            ...
            this.filteredMessages = this.allMessages.filter(msg => {
                const matchesSearch = !searchTerm ||
                    ...
                    (looksLikePhone &&
                        (PhoneValidator.matches(msg.from, searchTerm) ||
                         PhoneValidator.matches(msg.to, searchTerm)));
    ...
    exportToCSV(messages, prefix) {
        const headers = ['Date Sent', 'From', 'To', 'Direction', 'Status', 'Segments', 'Price', 'Body', 'Error Code', 'Error Message'];
